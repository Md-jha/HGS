...
namespace HGS.Utilites
{
    public interface IBasicUtility
    {
        ...
        string GenrateContactEmailTamplet(Contact data);
    }
    public class BasicUtility : IBasicUtility
    {
        ...
        [Obsolete]
        public string GenrateContactEmailTamplet(Contact data)
        {
            var htmlFilePath = _env.WebRootFileProvider.GetFileInfo("/EmailHtmlTemplate/GenrateContactEmailTamplet.html")?.PhysicalPath;
            var fileContent = System.IO.File.ReadAllText(htmlFilePath);
            var firstname = fileContent.Replace("rolling", data.firstname);
			var lastname = firstname.Replace("stone", data.lastname);
			var Email = lastname.Replace("mk@gmail.com", data.Email);
			var Phone = Email.Replace("799", data.Phone);
			var Company = Phone.Replace("hgs", data.Company);
			var subject = Company.Replace("Property", data.subject);
            var message = subject.Replace("message", data.message);
